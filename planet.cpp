...
bool planet::outline(int face, int x, int y) const
{
    if (sea(face, x, y) == 0)
    {
        globepoint point = getglobepoint(itsedge, face, x, y, 0, -1);

        if (point.face != -1)
        {
            if (sea(point.face, point.x, point.y) == 1)
                return 1;
        }

        point = getglobepoint(itsedge, face, x, y, 0, 1);
        ...
        point = getglobepoint(itsedge, face, x, y, -1, 0);
...
bool planet::coast(int face, int x, int y) const
{
    /*
    if (sea(face, x, y) == 1)
    {
...
void planet::longitude(int face, int x, int &degrees, int &minutes, int &seconds, bool& negative) const
{
    float fx = (float)x;
...
void planet::latitude(int face, int y, int& degrees, int& minutes, int& seconds, bool &negative) const
{
...
void planet::clear()
{
    for (int i = 0; i < ARRAYWIDTH; i++) // Set all the maps to 0.
    {
        for (int j = 0; j < ARRAYWIDTH; j++)
        {
            ...
            {
                mapnom[face][i][j] = 0;
                jantempmap[face][i][j] = 0;
                jultempmap[face][i][j] = 0;
                climatemap[face][i][j] = 0;
                janrainmap[face][i][j] = 0;
                julrainmap[face][i][j] = 0;
                janmountainrainmap[face][i][j] = 0;
                julmountainrainmap[face][i][j] = 0;
                janmountainraindirmap[face][i][j] = 0;
                julmountainraindirmap[face][i][j] = 0;
                seatempreducemap[face][i][j] = 0;
                rivermapdir[face][i][j] = 0;
                rivermapjan[face][i][j] = 0;
                rivermapjul[face][i][j] = 0;
                windmap[face][i][j] = 0;
                lakemap[face][i][j] = 0;
                roughnessmap[face][i][j] = 0;
                mountainridges[face][i][j] = 0;
                mountainheights[face][i][j] = 0;
                craterrims[face][i][j] = 0;
                cratercentres[face][i][j] = 0;
                tidalmap[face][i][j] = 0;
                riftlakemapsurface[face][i][j] = 0;
                riftlakemapbed[face][i][j] = 0;
                specials[face][i][j] = 0;
                extraelevmap[face][i][j] = 0;
                deltamapdir[face][i][j] = 0;
                deltamapjan[face][i][j] = 0;
                deltamapjul[face][i][j] = 0;
                oceanridgemap[face][i][j] = 0;
                oceanridgeheightmap[face][i][j] = 0;
                oceanriftmap[face][i][j] = 0;
                oceanridgeoffsetmap[face][i][j] = 0;
                islandmap[face][i][j] = 0;
                noshademap[face][i][j] = 0;
                mountainislandmap[face][i][j] = 0;
                volcanomap[face][i][j] = 0;
                stratomap[face][i][j] = 0;
                cloudsmap[face][i][j] = 0;
                noisemap[face][i][j] = 0;
                testmap[face][i][j] = 0;
            }
        }
...
void planet::setmaxriverflow()
{
    ...
    for (int face = 0; face < 6; face++)
    {
        for (int i = 0; i < itsedge; i++)
        {
            for (int j = 0; j < itsedge; j++)
            {
                current = riveraveflow(face, i, j);
...
void planet::saveworld(string filename)
{
    ...
    writevariable(outfile, itsambient);
    ...
    writevariable(outfile, itssea1);
    writevariable(outfile, itssea2);
    writevariable(outfile, itssea3);
    ...
    writevariable(outfile, itsatmos1);
    writevariable(outfile, itsatmos2);
    writevariable(outfile, itsatmos3);
    writevariable(outfile, itsdusk1);
    writevariable(outfile, itsdusk2);
    writevariable(outfile, itsdusk3);
    writevariable(outfile, itssun1);
    writevariable(outfile, itssun2);
    writevariable(outfile, itssun3);
    ...
    writedata(outfile, seatempreducemap);
...
bool planet::loadworld(string filename)
{
    ...
    readvariable(infile, itsambient);
    ...
    readvariable(infile, itssea1);
    readvariable(infile, itssea2);
    readvariable(infile, itssea3);
    ...
    readvariable(infile, itsatmos1);
    readvariable(infile, itsatmos2);
    readvariable(infile, itsatmos3);
    readvariable(infile, itsdusk1);
    readvariable(infile, itsdusk2);
    readvariable(infile, itsdusk3);
    readvariable(infile, itssun1);
    readvariable(infile, itssun2);
    readvariable(infile, itssun3);
    ...
    readdata(infile, seatempreducemap);
...
void planet::smooth(int arr[][ARRAYWIDTH], int amount, bool vary, bool avoidmountains) // This smoothes the given array by the given amount.
{
    /*
...
void planet::smooth(short arr[][ARRAYWIDTH], int amount, bool vary, bool avoidmountains) // This smoothes the given array by the given amount.
{
    /*
    for (int i = 0; i <= itswidth; i++)
    {
...
void planet::smoothoverland(int arr[][ARRAYWIDTH][ARRAYWIDTH], int amount, bool uponly)
{
    for (int face = 0; face < 6; face++)
    {
        for (int i = 0; i < itsedge; i++)
        {
            for (int j = 0; j < itsedge; j++)
            {
                if (sea(face, i, j) == 0)
                {
                    ...
                    {
                        globepoint kpoint = getglobepoint(itsedge, face, i, j, k, 0);

                        if (kpoint.face != -1)
                        {
                            for (int l = -amount; l <= amount; l++)
                            {
                                globepoint lpoint = getglobepoint(itsedge, kpoint.face, kpoint.x, kpoint.y, 0, l);
                                
                                if (lpoint.face != -1)
                                {
                                    ave = ave + arr[lpoint.face][lpoint.x][lpoint.y];
                                    crount++;
                                }
                            }
                        }
...
void planet::smoothoverland(short arr[][ARRAYWIDTH][ARRAYWIDTH], int amount, bool uponly)
{
    for (int face = 0; face < 6; face++)
    {
        for (int i = 0; i < itsedge; i++)
...
template<typename T> void planet::shift(T arr[][ARRAYWIDTH], int offset)
{
    /*
    vector<vector<T>> dummy(ARRAYWIDTH, vector<T>(ARRAYWIDTH, 0));

    //T dummy[ARRAYWIDTH][ARRAYWIDTH];
...
template<typename T> void planet::writedata(ofstream& outfile, T const arr[ARRAYWIDTH][ARRAYWIDTH])
{
    for (int i = 0; i <= itswidth; i++)
    {
        for (int j = 0; j <= itsheight; j++)
...
template<typename T> void planet::readdata(ifstream& infile, T arr[ARRAYWIDTH][ARRAYWIDTH])
{
    /*
    for (int i = 0; i <= itswidth; i++)
    {
...
simpleplanet::simpleplanet() //constructor
{
...
void simpleplanet::clear()
{
    for (int i = 0; i < ARRAYWIDTH * 4; i++) // Set all the maps to 0.
    {
        for (int j = 0; j < ARRAYWIDTH * 4; j++)
        {
            for (int face = 0; face < 6; face++)
            {
                mapnom[face][i][j] = 0;
